...
\begin{figure}[p]
  ...
  \begin{subfigure}[b]{0.52\textwidth}
    ...
    \caption{Normalized contributions to total loss: inverse-width regularizer (\texttt{inv\_alpha}), physics loss enforcing energy conditions (\texttt{physics\_loss}), interface mismatch penalty (\texttt{reg\_breach}), and their sum (\texttt{total\_loss}) as functions of epoch.}
  ...
  \begin{subfigure}[b]{0.52\textwidth}
    ...
    \caption{Fraction of grid points satisfying NEC/WEC/DEC/SEC; SEC remains the easiest condition to satisfy, DEC remains the tightest global constraint, and NEC/WEC settle at intermediate but still significantly constrained levels.}
  ...
  \begin{subfigure}[b]{0.52\textwidth}
    ...
    \caption{Convergence of seed parameters $A$ and $B$ for the single-shell seed; the main run remains almost stationary, consistent with a pretraining stage that already selected a near-plateau configuration before the published optimization history begins.}
    \label{fig:d1-params-epoch}
  \end{subfigure}

  \caption{Optimization diagnostics for single-shell configuration at $v=0.1$, $t=30$.
  The main run stays in a narrow parameter band after pretraining, while the exported diagnostics settle into a constrained regime with SEC above NEC/WEC and DEC as the tightest residual condition.}
  \label{fig:d1-opt}
\end{figure}
...
\begin{figure}[p]
  ...
  \begin{subfigure}[b]{0.56\textwidth}
    ...
    \caption{Normalized loss components for the double shell: \texttt{inv\_alpha}, \texttt{physics\_loss}, \texttt{reg\_breach}, and \texttt{total\_loss}, with the same interpretation as in Fig.~\ref{fig:d1-loss-components}.}
  ...
  \begin{subfigure}[b]{0.56\textwidth}
    ...
    \caption{Fraction of grid points satisfying NEC/WEC/DEC/SEC; the double-shell case settles into a constrained regime in which SEC remains the easiest condition to satisfy, NEC/WEC stay above DEC, and DEC remains the tightest global constraint.}
  ...
  \begin{subfigure}[b]{0.56\textwidth}
    ...
    \caption{Convergence of seed parameters $A$, $B$, and $R_0$; after a modest early adjustment the main run remains in a narrow parameter band, indicating a stable double-shell configuration within the adopted loss and diagnostic choices.}
    \label{fig:d2-params-epoch}
  \end{subfigure}

  \caption{Optimization diagnostics for double-shell configuration at $v=0.1$, $t=30$.
  Compared to Fig.~\ref{fig:d1-opt}, the same diagnostics shows that the optimizer finds a viable but more strongly constrained solution, with persistent DEC-limited regions near the inner interface.}
  \label{fig:d2-opt}
\end{figure}
